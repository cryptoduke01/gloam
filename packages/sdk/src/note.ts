/**
 * Poseidon note scheme. Matches the circom circuits + NoteLibPoseidon.sol.
...
export async function randomSecretField(): Promise<bigint> {
  const bytes = new Uint8Array(31); // stay under the field
...
export async function makeBoundNotePoseidon(
  ...
  const commitmentField = await noteCommitmentPoseidon(secretField, amount, asset);
  const nullifierField = await noteNullifierPoseidon(secretField, commitmentField);
  return {
